  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-slate-600">
              Factura {data.invoiceNumber} · {data.supplier}
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            <Link
              href={`/importaciones/${data.id}/editar`}
              className="rounded-2xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-900 hover:bg-slate-50"
            >
              Editar
            </Link>
            <Link
              href={`/importaciones/${data.id}/gastos/nuevo`}
              className="rounded-2xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-900 hover:bg-slate-50"
            >
              Agregar gasto
            </Link>
            <Link
              href={`/importaciones/${data.id}/produccion`}
            ...
            <form action={deleteImport}>
              <button className="rounded-2xl bg-rose-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-rose-700">
                Eliminar
              </button>
            </form>
          </div>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        <Card
          title="Subtotal mercancía"
          value={formatUsd(Number(data.subtotalGoodsUsd))}
          subtitle={`Factura ${formatDate(data.invoiceDate)}`}
        />
        <Card
          title="Flete internacional"
          value={formatUsd(Number(data.freightUsd))}
          subtitle={data.freightProrationMethod}
        />
        <Card
          title="Total importación"
          value={formatUsd(Number(data.importTotalUsd))}
          subtitle="Mercancía + flete"
        />
        <Card
          title="Gastos adicionales"
          value={formatUsd(Number(data.totalAdditionalExpenses))}
          subtitle={`${data.expenses.length} gastos`}
        />
        <Card
          title="Total invertido"
          value={formatUsd(Number(data.totalInvestedUsd))}
          subtitle="Costo consolidado"
        />
      </section>

      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        <Card
          title="Peso importado"
          value={`${Number(data.totalNetWeightKg).toFixed(2)} kg`}
          subtitle="Peso neto"
        />
        <Card
          title="Peso producido"
          value={`${producedWeightKg.toFixed(2)} kg`}
          subtitle="Teórico exportable"
        />
        <Card
          title="Desperdicio"
          value={`${wasteKg.toFixed(2)} kg`}
          subtitle="Final"
        />
        <Card
          title="Unidades confeccionadas"
          value={String(producedUnits)}
          subtitle="Producción total"
        />
        <Card
          title="Unidades vendibles"
          value={String(sellableUnits)}
        ...
        tabs={[
            ...
            content: (
              <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-soft">
                <div className="border-b border-slate-200 px-6 py-5">
                  <h2 className="text-xl font-semibold text-slate-950">Ítems importados</h2>
                </div>
                  ...
                  <table className="min-w-full text-sm">
                    <thead className="bg-slate-50 text-left text-slate-500">
                      <tr>
                        ...
                        <th className="px-6 py-4">Costo/u conf.</th>
                        <th className="px-6 py-4">Costo/u vend.</th>
                        <th className="px-6 py-4">Costo/kg</th>
                        <th className="px-6 py-4">Desperdicio</th>
                      </tr>
                    </thead>
                    <tbody>
                      {data.items.map((item) => {
                        ...
                        return (
                          <tr key={item.id} className="border-t border-slate-100">
                            ...
                            <td className="px-6 py-4">
                              {snapshot ? pct(Number(snapshot.participationByValuePct)) : '-'}
                            </td>
                            <td className="px-6 py-4">
                              {snapshot ? formatUsd(Number(snapshot.totalItemCostUsd)) : '-'}
                            </td>
                            <td className="px-6 py-4">
                              {snapshot?.costPerProducedUnitUsd
                                ? formatUsd(Number(snapshot.costPerProducedUnitUsd))
                                : 'Pendiente'}
                            </td>
                            <td className="px-6 py-4">
                              {snapshot?.costPerSellableUnitUsd
                                ? formatUsd(Number(snapshot.costPerSellableUnitUsd))
                                : 'Pendiente'}
                            </td>
                            <td className="px-6 py-4">
                              {snapshot?.costPerExportableKgUsd
                                ? formatUsd(Number(snapshot.costPerExportableKgUsd))
                                : 'Pendiente'}
                            </td>
                            <td className="px-6 py-4">
                              {item.production
                                ? `${Number(item.production.finalWasteKg).toFixed(2)} kg`
                                : 'Pendiente'}
                            </td>
                          </tr>
                        )
          ...
          },
          {
            ...
            content: (
              <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-soft">
                <div className="border-b border-slate-200 px-6 py-5">
                  <h2 className="text-xl font-semibold text-slate-950">Gastos y prorrateo</h2>
                </div>
                <div className="overflow-x-auto">
                  <table className="min-w-full text-sm">
                    <thead className="bg-slate-50 text-left text-slate-500">
                      ...
                      {data.expenses.map((expense) => {
                        ...
                        return (
                          <tr key={expense.id} className="border-t border-slate-100">
                            ...
                            <td className="px-6 py-4">{expense.prorationMethod}</td>
                            <td className="px-6 py-4">{formatUsd(Number(expense.amountUsd))}</td>
                            <td className="px-6 py-4">
                              {expense.paymentStatus === 'PAID' ? 'Pagado' : 'Pendiente'}
                            </td>
                            <td className="px-6 py-4">{expense.allocations.length} asignaciones</td>
                            <td className="px-6 py-4">
                              <form action={remove}>
                    ...
                    <div className="mt-3 space-y-1 text-sm text-slate-600">
                      <p>
                        ...
                        <span className="font-medium text-slate-900">
                          {item.production?.producedUnits ?? 0}
                        </span>
                      </p>
                      <p>
                        Vendibles:{' '}
                        <span className="font-medium text-slate-900">
                          {item.production?.sellableUnits ?? 0}
                        </span>
                      </p>
                      <p>
                        Peso teórico:{' '}
                        <span className="font-medium text-slate-900">
                          {Number(item.production?.theoreticalExportWeightKg ?? 0).toFixed(2)} kg
                        </span>
                      </p>
                      <p>
                        Desperdicio final:{' '}
                        <span className="font-medium text-slate-900">
                          {Number(item.production?.finalWasteKg ?? 0).toFixed(2)} kg
                        </span>
                      </p>
                    </div>
                  </article>
                  ...
                  {data.snapshots.map((snapshot) => (
                    <div key={snapshot.id} className="rounded-xl border border-slate-200 p-4">
                      <div className="flex items-center justify-between">
                        <p className="font-semibold">Versión {snapshot.version}</p>
                        <p className="text-xs text-slate-500">{formatDate(snapshot.createdAt)}</p>
                      </div>
                      <p className="text-sm text-slate-500">
                        {snapshot.reason ?? 'Recálculo automático'}
                      </p>
                      <p className="text-sm text-slate-500">
                        Antes: {snapshot.previousValues ? JSON.stringify(snapshot.previousValues) : 'N/A'} | Después:{' '}
                        {snapshot.newValues ? JSON.stringify(snapshot.newValues) : 'N/A'}
                      </p>
                    </div>
                  ))}
                </div>
